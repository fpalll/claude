...
# OpenAI client always sends Authorization: Bearer <key>. An empty key becomes "Bearer ",
# which h11 rejects (LocalProtocolError: Illegal header value b'Bearer ').
# Local OpenAI-compatible servers ignore a dummy token; set OPENAI_API_KEY if yours does not.
API_KEY = (os.environ.get("OPENAI_API_KEY") or "").strip() or "not-needed"
args = None
...
def debug_print_model_output(question_num, each, final_pred, final_response, final_raw):
    """Emit each completed generation to stderr (does not corrupt Rich Live on stdout)."""
    if args is None or not getattr(args, "debug_print_output", False):
        return
    qid = each.get("question_id", "?")
    out = []
    out.append("")
    out.append("=" * 72)
    out.append(
        f"[debug] Q{question_num} question_id={qid} "
        f"pred={final_pred!r} gold={each.get('answer')!r}"
    )
    out.append("-" * 72)
    out.append("" if final_response is None else str(final_response))
    if final_raw is not None and str(final_raw) != str(final_response):
        out.append("-" * 72 + " [model_outputs_raw]")
        out.append(str(final_raw))
    out.append("=" * 72)
    sys.stderr.write("\n".join(out) + "\n")
    sys.stderr.flush()
...
def _loopback_ipv4_base_url(url: str) -> str:
    """Use 127.0.0.1 instead of localhost/::1 so httpx does not hit ::1 when the
    server only listens on IPv4 (intermittent APIConnectionError under load)."""
    try:
        p = urlparse(url)
    except Exception:
        return url
    ...
    if host not in ("localhost", "::1"):
        return url
    netloc = f"127.0.0.1:{p.port}" if p.port else "127.0.0.1"
    return urlunparse((p.scheme, netloc, p.path, p.params, p.query, p.fragment))


def _format_api_exception(e: BaseException, max_len: int = 400) -> str:
    ...
    if len(parts) == 1:
        ...
        if ctx is not None and ctx is not e:
            parts.append(f"{type(ctx).__name__}: {ctx}")
    return " | ".join(parts)[:max_len]


def get_async_client():
    base_url = _loopback_ipv4_base_url(args.url)
    kw = dict(api_key=API_KEY, base_url=base_url, timeout=3600.0)
    # trust_env=False: HTTP(S)_PROXY must not send loopback traffic through a proxy
    # (common cause of generic "Connection error" to local inference servers).
    # http2=False + max_keepalive_connections=0: many concurrent streams to vLLM/SGLang
    # often trigger h11 LocalProtocolError ("Illegal status line") on reused sockets.
    if _is_loopback_api_url(args.url):
...
def apply_debug_sample(test_data, sample_size, sample_seed):
    """Return a subset of test_data for quick debug runs.

    If ``sample_size`` is None or <= 0, returns ``test_data`` unchanged.
    If ``sample_size`` >= len(test_data), returns full list.
    If ``sample_seed`` is None, returns the first ``sample_size`` items (stable order).
    Otherwise returns ``sample_size`` items chosen with ``random.Random(sample_seed)``.
    """
    ...
    idxs = rng.sample(range(n), sample_size)
    idxs.sort()
    return [test_data[i] for i in idxs]
...
def output_from_message_content_or_reasoning(content: str, reasoning: str) -> str:
    """Match chat.completions message fields: msg['content'] or msg.get('reasoning_content','')."""
    return content if content else (reasoning or "")
...
class BadFinishReasonError(Exception):
    """Stream ended with finish_reason other than ``stop`` (e.g. length, content_filter)."""

    def __init__(self, reason):
        self.reason = reason
        super().__init__(
            f"finish_reason is {reason!r}, expected 'stop'"
        )


class EmptyStreamResponseError(Exception):
    """Stream produced no concatenated model text (null / empty deltas)."""
...
class ActiveQuestionTracker:
    def __init__(self, subject, total_questions, max_concurrent=4, loop=None):
        ...
        tr = getattr(args, "transient_retries", 15)
        if tr > 0:
            self.log(f"Max transient (API/stream) attempts per question: {tr}")
        else:
            self.log(
                "Transient API/stream attempts: unlimited (--transient-retries 0)"
            )
...
async def streaming_request(
    ...
    async def _streaming_body():
        nonlocal stream, total_tokens, usage_data
        full_response_parts = []
        raw_parts = []
        content_parts = []
        reasoning_parts = []
        ...
        if getattr(args, "no_thinking", False):
            # vLLM / many OpenAI-compatible Qwen3 servers (see vLLM --default-chat-template-kwargs)
        ...
        async for chunk in stream:
            ...
            # Final chunk sometimes has choice.message populated when delta is empty.
            if msg_obj is not None and not content and not thinking:
            ...
            if combined_text:
                raw_parts.append(combined_text)
        ...
        if args.save_thinking:
            ...
            # Same as msg = choice.message; msg["content"] or msg.get("reasoning_content","")
            response_text = output_from_message_content_or_reasoning(
                content_full, reasoning_full
            )

        if not raw_text.strip():
            raise EmptyStreamResponseError(
                "Stream contained no model text (null or empty deltas)"
            )

        # If both message fields stayed empty but chronologic raw stream has text
        # (unusual chunk shapes), fall back to raw_text for extraction/saving.
        if not response_text.strip() and raw_text.strip():
            response_text = raw_text

        relaxed = getattr(args, "allow_missing_finish_reason", False)
        ...
        return response_text, total_tokens, raw_text, finish_reason

    try:
        # asyncio.timeout() is 3.11+; wait_for works on 3.10 (same 3600s cap).
        return await asyncio.wait_for(_streaming_body(), timeout=3600)
...
async def evaluate_subject_async(
    ...
    with Live(
        ...
        async def process_one(question_num, each):
            ...
            max_transient_retries = getattr(args, "transient_retries", 15)
                ...
                while True:
                    ...
                    if kill_flag:
                        ...
                        with file_lock:
                            ...
                            each["model_outputs_raw"] = None
                            each["finish_reason"] = None
                    ...
                    try:
                    ...
                    except EmptyStreamResponseError as e:
                        ...
                        if (
                            ...
                            tracker.log(
                                f"Q{question_num} empty/null stream — giving up after "
                                f"{max_transient_retries} transient attempts ({e})"
                            )
                            final_response = (
                                f"EmptyStreamResponseError after {max_transient_retries} attempts"
                            )
                            request_success = False
                            break
                        tracker.log(f"Q{question_num} empty/null stream — retry ({e})")
                        tracker.increment_retry_count(question_num)
                        tracker.update_token(question_num, 0)
                        await asyncio.sleep(2)
                        continue

                    except BadFinishReasonError as e:
                        transient_attempts += 1
                        if (
                            max_transient_retries > 0
                            and transient_attempts >= max_transient_retries
                            ...
                            tracker.log(
                                f"Q{question_num} finish_reason={e.reason!r} — giving up after "
                                f"{max_transient_retries} transient attempts"
                            )
                            final_response = (
                                f"BadFinishReasonError({e.reason!r}) after "
                                f"{max_transient_retries} attempts"
                            )
                            request_success = False
                            break
                        tracker.log(
                            f"Q{question_num} finish_reason={e.reason!r} (need 'stop') — retry"
                        )
                        tracker.increment_retry_count(question_num)
                        tracker.update_token(question_num, 0)
                        await asyncio.sleep(2)
                        ...
                        if (
                            ...
                            tracker.log(
                                f"Q{question_num} giving up after {max_transient_retries} "
                                f"transient attempts: {_format_api_exception(e)}"
                            )
                            final_response = (
                                f"Error after {max_transient_retries} attempts: "
                                f"{_format_api_exception(e, max_len=800)}"
                            )
                            request_success = False
                            break
                        tracker.log(f"Q{question_num} Err: {_format_api_exception(e)}")
                        tracker.increment_retry_count(question_num)
                        tracker.update_token(question_num, 0)
            ...
            with file_lock:
                ...
                each["model_outputs_raw"] = final_raw
                each["finish_reason"] = final_finish_reason
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=15,
        metavar="N",
        help="Max failed attempts per question for API errors, empty stream, bad "
        "finish_reason, missing usage, etc. 0 = unlimited (old behavior). "
        "MaxTokensExceeded is capped separately by --retry. Default: 15.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Disable Qwen3 extended thinking: send "
        "extra_body.chat_template_kwargs.enable_thinking=false (vLLM/OpenAI-compatible).",
    )
    parser.add_argument(
        "--debug-print-output",
        action="store_true",
        default=False,
        help="Print every completed model_outputs (and raw if different) to stderr for debugging.",
    )
    parser.add_argument(
        "--rerun-maxtoken",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help="Debug: evaluate at most N questions per subject (omit for full split). "
        "Without --sample-seed, uses the first N questions in order.",
    )
    parser.add_argument(
        "--sample-seed",
        type=int,
        default=None,
        metavar="S",
        help="Debug: with --sample-size, randomly pick N questions using seed S (reproducible).",
    )
    parser.add_argument(
        "--allow-missing-finish-reason",
        action="store_true",
        default=False,
        help="If the server omits finish_reason but returned non-empty text, accept the "
        "stream (default: require finish_reason == stop, else retry).",
    )
    ...
    if args.sample_size:
        if args.sample_seed is not None:
            console.print(
                f"[yellow]Debug sample: {args.sample_size} questions/subject "
                f"(random, seed={args.sample_seed})[/yellow]"
            )
        else:
            console.print(
                f"[yellow]Debug sample: first {args.sample_size} questions per subject[/yellow]"
            )
    if args.allow_missing_finish_reason:
        console.print(
            "[yellow]--allow-missing-finish-reason: accepting streams with no "
            "finish_reason if text is non-empty[/yellow]"
        )
    if args.no_thinking:
        console.print(
            "[yellow]--no-thinking: chat_template_kwargs enable_thinking=false "
            "(Qwen3 / vLLM-style servers)[/yellow]"
        )
    if args.debug_print_output:
        console.print(
            "[yellow]--debug-print-output: printing each generation to stderr[/yellow]"
        )
    ...
    for subject in assigned_subjects:
        ...
        test_data = apply_debug_sample(
            test_data, args.sample_size, args.sample_seed
        )
