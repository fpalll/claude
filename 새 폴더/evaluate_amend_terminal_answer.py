#!/usr/bin/env python3
# Based on evaluate_amend.py: if the completion does not end with "Answer: (A-J)"
# or "The answer is (<A-J>)", the same request is retried immediately until -r limit.
import os
import json
import re
import time
...
# Response must end with an explicit final-answer line (matches evaluation prompt).
_TERMINAL_ANSWER_END_RE = re.compile(
    r"(?:"
    r"[aA]nswer:\s*[A-J]"
...
def response_ends_with_answer_marker(text: str) -> bool:
    """True if stripped model text ends with 'Answer: X' or 'The answer is (X)' (A–J)."""
    if not text:
        return False
    return bool(_TERMINAL_ANSWER_END_RE.search(text.rstrip()))
...
class ActiveQuestionTracker:
    def __init__(self, subject, total_questions, max_concurrent=4, loop=None):
        ...
        if args.retry > 0:
            self.log(f"Max token / terminal-answer-format retries: {args.retry}")
    ...
    with Live(
        ...
        async def process_one(question_num, each):
            ...
            while wrong_attempts <= max_wrong_retries and not GLOBAL_QUIT_REQUESTED:
                ...
                terminal_answer_attempts = 0
                request_success = False
                response = None
                token_count = None
                skip_start_delay = False
                ...
                while True:
                    ...
                    try:
                        ...
                        if not response_ends_with_answer_marker(response):
                            terminal_answer_attempts += 1
                            tracker.increment_retry_count(question_num)
                            if terminal_answer_attempts >= max_token_retries:
                                tracker.log(
                                    f"Q{question_num} no terminal Answer:... after "
                                    f"{max_token_retries} retries; using response as-is"
                                )
                                request_success = True
                                break
                            tracker.log(
                                f"Q{question_num} output must end with "
                                f"Answer: (A-J) or The answer is (X); retrying"
                            )
                            tracker.update_token(question_num, 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Max retries for MaxTokensExceeded and for outputs missing terminal Answer:... / The answer is (X)",
    )
