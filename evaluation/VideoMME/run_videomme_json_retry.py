...
# Fork of run_videomme.py: --json-answer-retry / --json-answer-max-retries for OpenAI + qwen35
# MCQ — retry until the visible completion (after </redacted_thinking>) ends with a valid
# JSON-style "answer": "<letter>" with no trailing junk.
...
def _videomme_choice_letters(annotation: Dict[str, Any]) -> List[str]:
    ch = annotation.get("choices") or {}
    return sorted(ch.keys())


def _response_ends_with_qwen35_json_answer(text: str, valid_letters: List[str]) -> bool:
    """
    True if the post-thinking visible text ends with a JSON-style answer field for one
    allowed letter, with only optional whitespace / a single closing brace after it.
    """
    visible = str(text).split("</redacted_thinking>")[-1].strip()
    if not visible or visible.startswith("[ERROR]"):
        return False
    valid = {x.upper() for x in valid_letters}
    if not valid:
        return True
    s = visible.strip()
    matches = list(_JSON_ANSWER_FIELD_RE.finditer(s))
    if not matches:
        return False
    m = matches[-1]
    if m.group(1).upper() not in valid:
        return False
    tail = s[m.end() :].lstrip()
    if tail.startswith("}"):
        tail = tail[1:].lstrip()
...
def run_inference_openai(args):
    ...
    if getattr(args, "json_answer_retry", False):
        if getattr(args, "prompt_style", "default") != "qwen35":
            ...
            print(
                ...
                "per sample until visible output ends with a valid "
                '\"answer\": \"<letter>\" snippet (no trailing junk).\n'
            )
    ...
    def infer_one(
        ...
        valid_letters = _videomme_choice_letters(annotation)
        need_json_retry = (
        ...
        for attempt in range(max_tries):
            ...
            if not need_json_retry:
                break
            if _response_ends_with_qwen35_json_answer(response, valid_letters):
                break
...
def main():
    parser = argparse.ArgumentParser(
        description="VideoMME (json-retry fork): vLLM or OpenAI API; optional qwen35 JSON-answer retries"
    )
    ...
    infer_parser.add_argument(
        ...
        action="store_true",
        help=(
            "OpenAI + qwen35: retry generation until visible output ends with a valid "
            'JSON-style \"answer\": \"<letter>\" (see prompt hint)'
        ),
    )
    infer_parser.add_argument(
        ...
        type=int,
        default=8,
        help="Max attempts per sample when --json-answer-retry (default: 8)",
    )
    infer_parser.add_argument(
