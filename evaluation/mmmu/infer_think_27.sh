...
#   e.g., "answer": "C".
#
# Inference uses run_mmmu_json_retry.py (JSON "answer" retry for qwen35 MCQ).
# Sampling: max_tokens=81920, temperature=1.0, top_p=0.95, presence_penalty=1.5,
#           extra_body.top_k=20 (see run_mmmu_json_retry.py run_inference_openai).
#
...
#
export HF_HUB_CACHE=/data/winglam/hf/hub
export HF_DATASETS_CACHE=/data/winglam/hf/datasets
...
API_BASE="${API_BASE:-http://localhost:50037}"
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-qwen3_5-27b}"
...
OUTPUT_FILE="${OUTPUT_FILE:-${SCRIPT_DIR}/results/new/mmmu_predictions.jsonl}"
...
python run_mmmu.py infer \
  ...
  # --json-answer-retry \
  # --json-answer-max-retries 8

echo "Wrote predictions to ${OUTPUT_FILE}"

# --- Optional: judge / accuracy (same server can be used as the extractor judge) ---
export MIT_SPIDER_URL='http://localhost:50037/v1/chat/completions'
export MIT_SPIDER_TOKEN='EMPTY'
python run_mmmu.py eval \
  --data-dir "${DATA_DIR}" \
  --input-file "${OUTPUT_FILE}" \
  --output-file "${SCRIPT_DIR}/results/new/evaluation.csv" \
  --dataset MMMU_DEV_VAL \
  --eval-model "${SERVED_MODEL_NAME}" \
  --api-type mit \
  --nproc 16
# nohup bash infer_think_27.sh > new.log 2>&1 < /dev/null & 634866