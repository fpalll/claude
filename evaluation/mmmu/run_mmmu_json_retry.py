...
#
# Fork of run_mmmu.py: adds --json-answer-retry / --json-answer-max-retries for
# OpenAI + qwen35 MCQ (retry until output contains "answer": "<letter>").
...
_JSON_ANSWER_FIELD_RE = re.compile(
    r'["\']answer["\']\s*:\s*["\']([A-Za-z])["\']',
    re.IGNORECASE,
)


def _mmmu_mc_choice_letters(line_dict: Dict[str, Any]) -> List[str]:
    """Uppercase option keys (A, B, ...) present on this MMMU row."""
    letters: List[str] = []
...
def _response_has_qwen35_json_answer(text: str, valid_letters: List[str]) -> bool:
    """
    True if the model output contains a JSON-style answer field with one allowed letter,
    e.g. "answer": "C" or 'answer': 'C' (matches the qwen35 prompt hint).
    """
    if not text or str(text).strip().startswith("[ERROR]"):
        return False
    valid = {x.upper() for x in valid_letters}
    if not valid:
        return True
    for m in _JSON_ANSWER_FIELD_RE.finditer(str(text)):
...
def run_inference_openai(args):
    ...
    if getattr(args, "json_answer_retry", False):
        if getattr(args, "prompt_style", "default") != "qwen35":
            print(
                "Warning: --json-answer-retry only applies with --prompt-style qwen35 "
                f"(current: {getattr(args, 'prompt_style', 'default')}).\n"
            )
        else:
            print(
                f"JSON answer retry enabled: up to {args.json_answer_max_retries} attempts "
                'per multi-choice item until output contains a valid '
                '\"answer\": \"<letter>\" snippet.\n'
            )
    ...
    def infer_one(
        ...
        valid_letters = _mmmu_mc_choice_letters(line_dict)
        need_json_retry = (
            bool(getattr(args, "json_answer_retry", False))
            and getattr(args, "prompt_style", "default") == "qwen35"
            and len(valid_letters) > 0
        )
        max_tries = (
            max(1, int(getattr(args, "json_answer_max_retries", 1)))
            if need_json_retry
            else 1
        )
        ...
        for attempt in range(max_tries):
            ...
            if not need_json_retry:
                break
            if _response_has_qwen35_json_answer(response, valid_letters):
                break
            # Format (or empty / API error string): retry immediately with same prompt.
            if attempt >= max_tries - 1:
                break

        return line_dict, messages, response
    ...
    results = []
    for line_dict, messages, response in results_rows:
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "MMMU evaluation (JSON-answer retry variant): vLLM or OpenAI-compatible API; "
            "see run_mmmu.py for the baseline script."
        )
    )
    ...
    infer_parser.add_argument(
        ...
        type=float,
        default=600.0,
        help="Per-request timeout in seconds for --api-base (default: 600)",
    )
    infer_parser.add_argument(
        "--debug-errors",
        action="store_true",
    ...
    infer_parser.add_argument(
        ...
        action="store_true",
        help=(
            "OpenAI API only: for --prompt-style qwen35 and multi-choice items, "
            "repeat the same request until the output contains a JSON-style "
            '\"answer\": \"<letter>\" (letter must be a valid option) or '
            "--json-answer-max-retries is reached."
        ),
    )
    infer_parser.add_argument(
        "--json-answer-max-retries",
        type=int,
        default=8,
        help="Max completion attempts per sample when --json-answer-retry is set (default: 8).",
    )
