...
Exact settings from Qwen3.5-27B model card:
  ...
  - Shot:      0-shot
  - Dataset:   lmms-lab/Video-MME  (test split)

Frame extraction matches VLMEvalKit Qwen3-VL model.py:
  fps=2, max nframe=128  (NOT fixed 16/32/64 — that was wrong)

Prompt format matches VLMEvalKit vlmeval/vlm/qwen3_vl/prompt.py:
  - No system prompt
  - Frames first, then question + options
  - "Answer with the option letter only."

Generation params: temperature=0.01, top_p=0.8, top_k=20, presence_penalty=1.5
"""
import io
import json
import os
...
# Matches VLMEvalKit Qwen3-VL model.py: fps=2, nframe=128
TARGET_FPS = 2
MAX_FRAMES = 128


def _extract_frames_at_fps(video_bytes: bytes, fps: float = TARGET_FPS,
                            max_frames: int = MAX_FRAMES) -> list[Image.Image]:
    """Extract frames at a fixed fps rate, capped at max_frames."""
    # decord (fast, GPU-accelerated if available)
    try:
        import decord
        ...
        total_frames = len(vr)
        # Sample every (video_fps / target_fps) frames
        step = max(1, round(video_fps / fps))
        indices = list(range(0, total_frames, step))
        # Cap at max_frames with uniform subsampling if needed
...
def _build_question_content(
    ...
    text_parts.append(f"{question_text}\n{options_str}\nAnswer with the option letter only.")
    return frame_content + [{"type": "text", "text": "\n".join(text_parts)}]


def _run_pass(items, model, client, with_subs, workers, desc):
    def eval_video(idx: int, item: dict):
        video_bytes = _get_video_bytes(item)
        if not video_bytes:
            return None

        frames = _extract_frames_at_fps(video_bytes, fps=TARGET_FPS, max_frames=MAX_FRAMES)
        if not frames:
            return None
        ...
        for q in questions:
            ...
            q_results.append({
                ...
                "duration": str(item.get("duration_category", item.get("duration", ""))),
                "gold": gold,
                "pred": pred,
                "correct": pred == gold,
                "response": response,
...
def _stats(results: list, label: str, target: float) -> dict:
    ...
    print(f"Avg frames extracted: {avg_frames:.1f}  (fps={TARGET_FPS}, max={MAX_FRAMES})")
...
def run_videomme(
    ...
    thinking: bool = True,   # kept for API compat; thinking is server-side
    workers: int = 8,
) -> float:
    print("\n[Video-MME] Loading dataset (test split)...")
    ...
    if sample:
        ...
        for it in items:
            by_dur[str(it.get("duration_category", it.get("duration", "")))].append(it)
    ...
    print(f"\n[Video-MME] Without subtitles (fps={TARGET_FPS}, max_frames={MAX_FRAMES})...")
    no_sub_results = _run_pass(items, model, client, with_subs=False,
                               workers=workers, desc="Video-MME (no subs)")

    print(f"\n[Video-MME] With subtitles (fps={TARGET_FPS}, max_frames={MAX_FRAMES})...")
    sub_results = _run_pass(items, model, client, with_subs=True,
