...
OUTPUT_FILE="${OUTPUT_FILE:-${SCRIPT_DIR}/results/debug/_27b.jsonl}"
...
RUN_EVAL="${RUN_EVAL:-1}"
EVAL_OUTPUT_FILE="${EVAL_OUTPUT_FILE:-${SCRIPT_DIR}/results/debug/videomme_short_wo_subtitle_qwen35_evaluation_27b_retry.csv}"
EVAL_NPROC="${EVAL_NPROC:-16}"

if [[ "${RUN_EVAL}" != "0" ]]; then
  export MIT_SPIDER_URL="${MIT_SPIDER_URL:-http://localhost:50037/v1/chat/completions}"
...
# nohup bash infer_think.sh > 27bvideo.log 2>&1 < /dev/null & 656863