...
OUTPUT_FILE="${OUTPUT_FILE:-${SCRIPT_DIR}/results/debug/mmmu_predictions.jsonl}"
...
python run_mmmu.py eval \
  ...
  --output-file "${SCRIPT_DIR}/results/debug/evaluation.csv" \
  --dataset MMMU_DEV_VAL \
  --eval-model "${SERVED_MODEL_NAME}" \
  --api-type mit \
  --nproc 16
# nohup bash infer_think_27.sh > 27mmmu.log 2>&1 < /dev/null & 634866