...
def _openai_assistant_text(message: Dict[str, Any]) -> str:
    ...
    pieces: List[str] = []
    # Reasoning fields first, `content` last so a short final answer stays at the end.
    for key in (
        "reasoning_content",
        "reasoning",
        "thinking",
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        print(f"API error: unexpected response shape: {parse_err}")
                    ...
                    if not text:
                        if _eval_extraction_verbose():
                            print(
                                "API error: empty assistant text; message keys="
                                f"{list(msg.keys())} (retrying)"
                            )
                        else:
